...
#### Version 1
# This will be a continuation of version 0, with the following changes:
    # Take and average of every 2 rows to create 1 of them
        # Needed in order to turn 100Hz dataset into 50Hz
    # Create new features for the model to use during training
        # NOTE: this is NOT required, since CNNs act as a natural feat
                # extractor, according to research DeepConvLSTM paper
    # The version 0 shell commands all work, but the data
            #needs to be first averaged in order to
            # simulate 50hz from 100 hz
...
# df['x'] = df['x'].rolling(2, min_periods=2, step=2).mean(); df['y'] = df['y'].rolling(2, min_periods=2, step=2).mean(); df['z'] = df['z'].rolling(2, min_periods=2, step=2).mean()
## Then continue with rest of version 0
...
# #df.to_csv('cleanX_vY.csv', index=False)
